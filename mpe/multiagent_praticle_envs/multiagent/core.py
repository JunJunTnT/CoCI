...
#            agent.color = color
    ...
    def step(self, time=None):
        ...
        p_force = self.apply_action_force(p_force) # p_force is agent.action.u
    ...
    # integrate physical state
    def integrate_state(self, p_force, time=None):
        for i,entity in enumerate(self.entities):
            if not entity.movable: continue
#            p_vel_tem = np.copy(entity.state.p_vel)
            entity.state.p_vel = entity.state.p_vel * (1 - self.damping)
            if (p_force[i] is not None):
                entity.state.p_vel += (p_force[i] / entity.mass)* self.dt # soft move
#                entity.state.p_vel = (p_force[i] / entity.mass)*3*self.dt  # hard move * self.dt
            if entity.max_speed is not None:
                speed = np.sqrt(np.square(entity.state.p_vel[0]) + np.square(entity.state.p_vel[1]))
                if speed > entity.max_speed:
                    entity.state.p_vel = entity.state.p_vel / np.sqrt(np.square(entity.state.p_vel[0]) +
                                                                  np.square(entity.state.p_vel[1])) * entity.max_speed
            # entity.state.p_pos_list = []
            # for t in np.arange(0.02,self.dt,0.02):
            #     if t <= time[i]:
            #         pos = entity.state.p_pos+p_vel_tem*t  
            #     elif t <= self.dt:
            #         pos = entity.state.p_pos+p_vel_tem*min(self.dt,time[i])+entity.state.p_vel* max(0,(t-time[i]))
            #     entity.state.p_pos_list.append(np.clip(pos,-0.95,0.95))                               
            entity.state.p_pos += entity.state.p_vel* max(0,(self.dt-time[i]))
#           entity.state.p_pos += (p_vel_tem*min(self.dt,time[i]) + entity.state.p_vel* max(0,(self.dt-time[i])))
            entity.state.p_pos = np.clip(entity.state.p_pos,-0.95,0.95)
            # entity.state.p_pos_list.append(entity.state.p_pos)
#            entity.state.p_pos += entity.state.p_vel* max(0,(self.dt-time[i]))
#             print(entity.state.p_vel)
    ...
    def get_entity_collision_force(self, ia, ib):
        ...
        force = self.contact_force * delta_pos / max(0.001, dist) * penetration
