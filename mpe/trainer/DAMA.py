...
from trainer.common.replay_buffer import ReplayBuffer_D
...
def p_train(make_obs_ph_n, network_input, time_ph_n, time_space_n, p_timer, act_space_n, before_com_func, channel, after_com_func,
            q_func, optimizer, grad_norm_clipping=None, dim_message = 4, num_units=64, scope="trainer", reuse=None, beta=0.01):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        time_w_ph_n = [timer_w_pdtype_n[i].sample_placeholder([None], name="time_w"+str(i)) for i in range(num_agents)]

        hiddens_n = [before_com_func(tf.compat.v1.concat([obs_ph_n[i], network_input[i]], 1), dim_message, scope="before_com_{}".format(i), num_units=num_units) for i in range(num_agents)]
...
#        timer_n = [p_timer(tf.compat.v1.concat([obs_ph_n[i], network_input[i]], 1), 1, scope="timer_{}".format(i), num_units=int(num_units/2)) for i in range(num_agents)]
        timer_generate_n = [p_timer(tf.compat.v1.concat([obs_ph_n[i], network_input[i]], 1), num_agents, scope="timer_{}".format(i), num_units=num_units) for i in range(num_agents)]
...
#        get_timer= U.function(inputs= [obs_ph_n[p_index]]+ [network_input[p_index]], outputs=timer)
        ...
        for i in range(num_agents):
            ...
            messages_list_batch = tf.compat.v1.multiply(hiddens_n_for_message, time_matrix)# [num_agent, batch_size, num_agent, mess_dim]
            
            channel_output = channel(messages_list_batch, scope="channel"+str(i), num_heads=2) # [batch_size, num_agent, num_unints]
            ...
            p = after_com_func(tf.compat.v1.concat([hiddens_n[i], message_n[i]],1), int(act_pdtype_n[i].param_shape()[0]), scope="p_func_{}".format(i), num_units=num_units)
            p_n.append(p)
...
#            p_n_0.append(p_0)
        ...
        for i in range(num_agents):
            ...
            Z = tf.compat.v1.expand_dims(X,2)
            time_matrix = tf.compat.v1.tile(Z,(1,1,dim_message))
            messages_list_batch_ = tf.compat.v1.multiply(hiddens_n_for_message, time_matrix)# [num_agent, batch_size, num_agent, mess_dim]
            
            channel_output_ = channel(messages_list_batch_, scope="channel"+str(i), num_heads=2, reuse=True) # [batch_size, num_agent, num_unints]
            ...
            message_n_ = [tf.compat.v1.squeeze(message, axis=0) for message in tf.compat.v1.split(channel_output_, num_or_size_splits=num_agents, axis=0)]

            p2 = after_com_func(tf.compat.v1.concat([hiddens_n[i], message_n_[i]],1), int(act_pdtype_n[i].param_shape()[0]), scope="p_func_{}".format(i), num_units=num_units, reuse=True)
...
#            timer_input_n_n_timer_0[i][i][:,i]
        ...
        q_input_n_actor = [tf.compat.v1.concat(obs_ph_n + network_input + act_input_n_n_actor[i] + timer_input_n_n[i], 1) for i in range(num_agents)]
        q_n_actor = [q_func(q_input_n_actor[i], 1, scope="q_func_{}".format(i), reuse=True, num_units=num_units)[:,0] for i in range(num_agents)]
        ...
        q_input_n_timer = [tf.compat.v1.concat(obs_ph_n + network_input + act_input_n_n_timer[i] + timer_input_n_n_timer[i], 1) for i in range(num_agents)]
#        q_input_n_timer_0 = [tf.compat.v1.concat(obs_ph_n + network_input + act_input_n_n_timer_0[i] + timer_input_n_n_timer_0[i], 1) for i in range(num_agents)]
        q_n_t = [q_func(q_input_n_timer[i], 1, scope="q_func_{}".format(i), reuse=True, num_units=num_units)[:,0] for i in range(num_agents)]
...
#        [-tf.compat.v1.reduce_mean(qt-q0) for qt,q0 in zip( q_n_t, q_n_0)]
        ...
        train = U.function(inputs=obs_ph_n + act_ph_n + network_input + time_ph_n + time_w_ph_n, outputs=loss_actor, updates=[optimize_expr])
        train_timer = U.function(inputs=obs_ph_n + act_ph_n + network_input + time_ph_n + time_w_ph_n, outputs=loss_timer, updates=[optimize_expr_timer])
        ...
        target_hiddens_n = [before_com_func(tf.compat.v1.concat([obs_ph_n[i], network_input[i]], 1), dim_message, 
                                    scope="target_before_com_{}".format(i), num_units=num_units) for i in range(num_agents)]
        target_before_com_vars = [U.scope_vars(U.absolute_scope_name("target_before_com_{}".format(i))) for i in range(num_agents)]

        target_hiddens_n_for_message = tf.compat.v1.stack([before_com_func(tf.compat.v1.concat([obs_ph_n[i], network_input[i]], 1), dim_message,
...
#                                num_units=num_units) for i in range(num_agents)]
        ...
        T_timer_generate_n = [p_timer(tf.compat.v1.concat([obs_ph_n[i], network_input[i]], 1), num_agents, scope="target_timer_{}".format(i), num_units=num_units) for i in range(num_agents)]
        ...
        target_timer_vars = [U.scope_vars(U.absolute_scope_name("target_timer_{}".format(i))) for i in range(num_agents)]
        target_p_n = []
        for i in range(num_agents):
            ...
            target_messages_list_batch = tf.compat.v1.multiply(target_hiddens_n_for_message, target_time_matrix)# [num_agent, batch_size, num_agent, mess_dim]
            target_channel_output = channel(target_messages_list_batch, scope="target_channel"+str(i), num_heads=2) # [batch_size, num_agent, num_unints]
            ...
            p_ = after_com_func(tf.compat.v1.concat([target_hiddens_n[i], target_message_n[i]],1), int(act_pdtype_n[i].param_shape()[0]), scope="target_p_func_{}".format(i), num_units=num_units)
#            target_hiddens_n[i]+target_message_n[i] + target_timer_n[i]
            target_p_n.append(p_)
        target_channel_vars_n = [U.scope_vars(U.absolute_scope_name("target_channel"+str(i))) for i in range(num_agents)]
#        target_p_n = [after_com_func(target_hiddens_n[i]+target_message_n[i], int(act_pdtype_n[i].param_shape()[0]), scope="target_p_func_{}".format(i), num_units=num_units) for i in range(num_agents)]
        ...
        update_target_timer = make_update_exp(list(itertools.chain(*timer_vars)), list(itertools.chain(*target_timer_vars)))
        ...
        return act, train, train_timer, update_target_p, update_target_timer,{'p_values': p_values, 'target_act': target_act, 'check_message_n':check_message_n, 'check_hiddens_n': check_hiddens_n}


def q_train(make_obs_ph_n, network_input, time_space_n, act_space_n, q_func, optimizer, grad_norm_clipping=None, scope="trainer",
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        time_ph_n_w = [time_pdtype_n[i].sample_placeholder([None], name="time_w_{}".format(i)) for i in range(len(time_space_n))]
        ...
        # Create callable functions
        train = U.function(inputs=obs_ph_n + network_input + act_ph_n + time_ph_n_w + target_ph_n, outputs=loss, updates=[optimize_expr])
...
#            time_ph_n_w = timer_w_space_n,
        ...
        self.replay_buffer = ReplayBuffer_D(1e6)
...
#        return self.act(*obs)
    ...
    def action(self, obs_n, obsnet_n, Epi):
        ...
        return [act[0].tolist() for act in action_list[0]],  [act[0][0] for act in action_list[1]], [act[0].tolist() for act in action_list[2]]
