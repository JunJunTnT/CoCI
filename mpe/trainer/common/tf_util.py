...
def sum(x, axis=None, keepdims=False):
    return tf.compat.v1.reduce_sum(x, axis=None if axis is None else [axis], keep_dims = keepdims)
def mean(x, axis=None, keepdims=False):
    return tf.compat.v1.reduce_mean(x, axis=None if axis is None else [axis], keep_dims = keepdims)
def var(x, axis=None, keepdims=False):
    meanx = mean(x, axis=axis, keepdims=keepdims)
    return mean(tf.compat.v1.square(x - meanx), axis=axis, keepdims=keepdims)
def std(x, axis=None, keepdims=False):
    return tf.compat.v1.sqrt(var(x, axis=axis, keepdims=keepdims))
def max(x, axis=None, keepdims=False):
    return tf.compat.v1.reduce_max(x, axis=None if axis is None else [axis], keep_dims = keepdims)
def min(x, axis=None, keepdims=False):
    return tf.compat.v1.reduce_min(x, axis=None if axis is None else [axis], keep_dims = keepdims)
def concatenate(arrs, axis=0):
    return tf.compat.v1.concat(axis=axis, values=arrs)
def argmax(x, axis=None):
    return tf.compat.v1.argmax(x, axis=axis)
...
def is_placeholder(x):
    return type(x) is tf.compat.v1.Tensor and len(x.op.inputs) == 0
...
class tfInput(object):
    ...
    def get(self):
        """Return the tf.compat.v1 variable(s) representing the possibly postprocessed value
        of placeholder(s).
        """
...
class BatchInput(PlacholdertfInput):
    def __init__(self, shape, dtype=tf.compat.v1.float32, name=None):
...
class Uint8Input(PlacholdertfInput):
    def __init__(self, shape, name=None):
        ...
        super().__init__(tf.compat.v1.placeholder(tf.compat.v1.uint8, [None] + list(shape), name=name))
        self._shape = shape
        self._output = tf.compat.v1.cast(super().get(), tf.compat.v1.float32) / 255.0
...
def ensure_tf_input(thing):
    """Takes either tf.compat.v1.placeholder of tf.compat.v1Input and outputs equivalent tf.compat.v1Input"""
    if isinstance(thing, tf.compat.v1Input):
        return thing
    elif is_placeholder(thing):
        return PlacholdertfInput(thing)
    else:
        raise ValueError("Must be a placeholder or tf.compat.v1Input")
...
def minimize_and_clip(optimizer, objective, var_list, clip_val=10):
    ...
    if clip_val is None:
        ...
        for i, (grad, var) in enumerate(gradients):
            if grad is not None:
                gradients[i] = (tf.compat.v1.clip_by_norm(grad, clip_val), var)
...
def make_session(num_cpu):
    ...
    tf.compat.v1_config = tf.compat.v1.ConfigProto(
        inter_op_parallelism_threads=num_cpu,
        intra_op_parallelism_threads=num_cpu)
    return tf.compat.v1.Session(config=tf.compat.v1_config)
...
def ortho_init(scale=1.0):
    def _ortho_init(shape, dtype, partition_info=None):
        #lasagne ortho init for tf.compat.v1
        shape = tuple(shape)
        if len(shape) == 2:
            flat_shape = shape
...
def function(inputs, outputs, updates=None, givens=None):
    ...
    Parameters
    ----------
    inputs: [tf.compat.v1.placeholder or tf.compat.v1Input]
    ...
    def __init__(self, inputs, outputs, updates, givens, check_nan=False):
        for inpt in inputs:
            if not issubclass(type(inpt), tfInput):
                assert len(inpt.op.inputs) == 0, "inputs should all be placeholders of rl_algs.common.tf.compat.v1Input"
        self.inputs = inputs
        updates = updates or []
