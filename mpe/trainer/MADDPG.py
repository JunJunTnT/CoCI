...
def p_train(make_obs_ph_n, weights_ph, p_W, act_space_n,  p_func,
            q_func,  optimizer, com_type, grad_norm_clipping=None, local_q_func=False, num_units=64, scope="trainer", reuse=None):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        for i in range(num_agents):
            ...
            matrix = tf.compat.v1.tile(X, multiples=[4])
            ...
            Y2 = tf.compat.v1.tile([0], multiples=[2])
            Y2 = tf.compat.v1.cast(Y2, dtype=tf.compat.v1.float32)

            matrix2 = tf.compat.v1.tile(X, multiples=[2])
            matrix2 = tf.compat.v1.cast(matrix2, dtype=tf.compat.v1.float32)
            # Z = tf.compat.v1.concat([Y, matrix, Y2, matrix, Y2, Y2, matrix, Y2], 0)

            # Z2 = tf.compat.v1.concat([Y, Y, matrix2, Y, matrix2, matrix2, Y, matrix2], 0)
            Z2 = tf.compat.v1.concat([Y, Y, Y2, Y, Y2, Y2, Y, Y2], 0)

            # paket loss testing #
            # if np.random.random() < 0.25:
            #     matrix2 = tf.compat.v1.tile([0], multiples=[4])
            # else:
            #     matrix2 = tf.compat.v1.tile([1], multiples=[4])
            #
            # matrix2 = tf.compat.v1.cast(matrix2, dtype=tf.compat.v1.float32)
            # matrix = tf.compat.v1.multiply(matrix, matrix2)

            Z = tf.compat.v1.concat([Y, matrix, Y2, matrix, Y2, Y2, matrix, Y2], 0)
            # Z = tf.compat.v1.concat([Y, matrix, matrix2, matrix, matrix2, matrix2, matrix, matrix2], 0)
...
def q_train(make_obs_ph_n, weights, act_space_n,  q_func, optimizer, com_type, grad_norm_clipping=None,
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        for i in range(num_agents):
            #-------------hinder com.-------------------#
            # X = tf.compat.v1.greater_equal(weights[i][0], 0.0)  # 0.5 as gate
            # X = tf.compat.v1.cast(X, dtype=tf.compat.v1.float32)
            # matrix = tf.compat.v1.tile(X, multiples=[6])
            # matrix = tf.compat.v1.cast(matrix, dtype=tf.compat.v1.float32)
            # Y = tf.compat.v1.tile([1], multiples=[12])
            # Y = tf.compat.v1.cast(Y, dtype=tf.compat.v1.float32)
            # Z = tf.compat.v1.concat([Y, matrix], 0)
            #
            # if i < 4:
            #     p_n.append(tf.compat.v1.multiply(make_obs_ph_n[i], Z))
            # else:
            #     p_n.append(make_obs_ph_n[i])

            # ----------------hinder com2-----------#
            # X = tf.compat.v1.greater_equal(weights[i][0], 0.5)  # 0.5 as gate
            # X2 = tf.compat.v1.cast(X, dtype=tf.compat.v1.float32)
            # # X = tf.compat.v1.expand_dims(X2, 0)
            # matrix = tf.compat.v1.tile(X2, multiples=[4])
            # matrix = tf.compat.v1.cast(matrix, dtype=tf.compat.v1.float32)
            # matrix2 = tf.compat.v1.tile(X2, multiples=[20])
            # matrix2 = tf.compat.v1.cast(matrix2, dtype=tf.compat.v1.float32)
            #
            # Y = tf.compat.v1.tile([1], multiples=[8])
            # Y = tf.compat.v1.cast(Y, dtype=tf.compat.v1.float32)
            # Y2 = tf.compat.v1.tile([1], multiples=[4])
            # Y2 = tf.compat.v1.cast(Y2, dtype=tf.compat.v1.float32)
            # Y3 = tf.compat.v1.tile([0], multiples=[4])
            # Y3 = tf.compat.v1.cast(Y3, dtype=tf.compat.v1.float32)
            # Y4 = tf.compat.v1.tile([1], multiples=[6])
            # Y4 = tf.compat.v1.cast(Y4, dtype=tf.compat.v1.float32)
            # Y5 = tf.compat.v1.tile([1], multiples=[20])
            # Y5 = tf.compat.v1.cast(Y5, dtype=tf.compat.v1.float32)
            # Z = tf.compat.v1.concat([Y, Y2, matrix, Y2, matrix], 0)
            #------------random--------#

            # Z = tf.compat.v1.concat([Y, Y2, Y3, Y2, Y2], 0)
            #
            # if i < 4:
            #     p_n.append(tf.compat.v1.multiply(obs_ph_n[i], Z))
            # else:
            #     p_n.append(make_obs_ph_n[i])

            # Z = tf.compat.v1.concat([Y, Y2, matrix, Y2, Y3, matrix2], 0)
            # Z2 = tf.compat.v1.concat([Y, Y2, Y2, Y2, Y4, Y2, matrix2], 0)

            # Z = tf.compat.v1.concat([Y, Y2, Y2, Y2, Y3, Y5], 0)
            # Z2 = tf.compat.v1.concat([Y, Y2, Y2, Y2, Y4, Y2, Y5], 0)

            # if i < 4:
            #     p_n.append(tf.compat.v1.multiply(obs_ph_n[i], Z))
            # else:
            #     p_n.append(tf.compat.v1.multiply(obs_ph_n[i], Z2))

            # ----------------hinder com2 complex-----------#

            # X = tf.compat.v1.greater_equal(weights[i][0], 0.1) # 0.5 as gate
            # X2 = tf.compat.v1.greater_equal(weights[i][0], 0.4)  # 0.5 as gate
            #
            # X = tf.compat.v1.cast(X, dtype=tf.compat.v1.float32)
            # X2 = tf.compat.v1.cast(X2, dtype=tf.compat.v1.float32)
            # # X = tf.compat.v1.expand_dims(X2, 0)
            #
            # matrix = tf.compat.v1.tile(X, multiples=[2])
            # matrix = tf.compat.v1.cast(matrix, dtype=tf.compat.v1.float32)
            # matrix2 = tf.compat.v1.tile(X2, multiples=[2])
            # matrix2 = tf.compat.v1.cast(matrix2, dtype=tf.compat.v1.float32)
            #
            #
            #
            ...
            # Y2 = tf.compat.v1.tile([1], multiples=[4])
            # Y2 = tf.compat.v1.cast(Y2, dtype=tf.compat.v1.float32)
            # Z = tf.compat.v1.concat([Y, Y2, matrix, matrix2, Y2, matrix, matrix2], 0)

            #---------------conplex2----------#
            # Z = tf.compat.v1.concat([Y, Y2, matrix, matrix, Y2, matrix2, matrix2], 0)
            ...
            # ----------------improve com complex-----------#
            # X = tf.compat.v1.greater_equal(weights[i][0], 0.33) # 0.5 as gate
            # X2 = tf.compat.v1.greater_equal(weights[i][0], 0.66)  # 0.5 as gate
            #
            # X = tf.compat.v1.cast(X, dtype=tf.compat.v1.float32)
            # X2 = tf.compat.v1.cast(X2, dtype=tf.compat.v1.float32)
            #
            ...
            # Y2 = tf.compat.v1.tile([0], multiples=[2])
            # Y2 = tf.compat.v1.cast(Y2, dtype=tf.compat.v1.float32)
            # Z = tf.compat.v1.concat([Y, matrix, matrix2, Y2, matrix, matrix2, Y2, Y2, matrix, matrix2, Y2], 0)
            # Z2 = tf.compat.v1.concat([Y, Y, Y2, Y, Y2, Y2, Y, Y2], 0)
            #
            # if i < 2:
            #     p_n.append(tf.compat.v1.multiply(make_obs_ph_n[i], Z))
...
class MADDPGAgentTrainer_C(AgentTrainer):
    def __init__(self, name, model, model_weights, obs_shape_n, act_space_n, args, lr, local_q_func=False,
                 size_buffer=1e6):
        self.name = name
        ...
        self.replay_buffer = ReplayBuffer_C(size_buffer)
...
#    def update_Timer(self, agents, t):        
...
#            target_act_next_n = [agents[i].p_debug['target_act'](obs_next_n[i]) for i in range(len(agents))]
