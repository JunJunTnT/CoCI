...
from trainer.common.replay_buffer import ReplayBuffer_W
...
#        get_timer= U.function(inputs= [obs_ph_n[p_index]]+ [network_input[p_index]], outputs=timer)
        ...
        sechdule_top, schedule_idx = tf.compat.v1.math.top_k(weigths2, k=capacity, sorted=False)
        kth = tf.compat.v1.reduce_min(sechdule_top, axis=1, keepdims=True)
        X = tf.compat.v1.greater_equal(weigths2, kth)
        ...
        for i in range(num_agents):
            ...
            messages_list_batch = [tf.compat.v1.squeeze(message, axis=1) for message in tf.compat.v1.split(messages_list_batch, num_or_size_splits=num_agents, axis=1)]
...
#            tf.compat.v1.reshape(messages_list_batch,[-1,num_agents ,dim_message])
            ...
            channel_output = channel(messages_list_batch, scope="channel"+str(i), num_heads=2) # [batch_size, num_agent, mess_dim]
            ...
            p = after_com_func(hiddens_n[i]+message_n[i], int(act_pdtype_n[i].param_shape()[0]), scope="p_func_{}".format(i), num_units=num_units)
            p_n.append(p)
...
#        p_n = [after_com_func(hiddens_n[i]+messages_list_batch[i], int(act_pdtype_n[i].param_shape()[0]), scope="p_func_{}".format(i), num_units=num_units) for i in range(num_agents)]
        ...
        w_q_n = [q_func(w_q_input_n[i], 1, scope="w_q_func_{}".format(i), reuse=True, num_units=num_units)[:,0] for i in range(num_agents)]
...
#        var_list.extend(weigths_vars)
        ...
        target_weigths = [p_W(obs_ph_n[i], 1, scope="target_weigths_{}".format(i),num_units=num_units) for i in range(num_agents)]
        ...
        sechdule_top, schedule_idx = tf.compat.v1.math.top_k(weigths2_, k=capacity, sorted=False)
        kth_ = tf.compat.v1.reduce_min(sechdule_top, axis=1, keepdims=True)
        X_ = tf.compat.v1.greater_equal(weigths2_, kth_)
        ...
        for i in range(num_agents):
            ...
            messages_list_batch_ = tf.compat.v1.multiply(target_hiddens_n_for_message, target_matrix)# [batch_size, num_agent, mess_dim]
            messages_list_batch_ = [tf.compat.v1.squeeze(message, axis=1) for message in tf.compat.v1.split(messages_list_batch_, num_or_size_splits=num_agents, axis=1)]
            ...
            target_channel_output = channel(messages_list_batch_, scope="target_channel"+str(i), num_heads=2) # [batch_size, num_agent, num_unints]
            ...
            p_ = after_com_func(target_hiddens_n[i]+target_message_n[i], int(act_pdtype_n[i].param_shape()[0]), scope="target_p_func_{}".format(i), num_units=num_units)
...
#        target_var_list.extend(target_weigths_vars)
        ...
        update_target_p_w = make_update_exp(list(itertools.chain(*weigths_vars)), list(itertools.chain(*target_weigths_vars)))
        ...
        return act, train, train_w, update_target_p, update_target_p_w, {'p_values': p_values, 'target_act': target_act, 'check_message_n':check_message_n, 'check_hiddens_n': check_hiddens_n}
...
#        act_ph_n = [act_pdtype_n[i].sample_placeholder([None], name="action_{}".format(i)) for i in range(len(act_space_n))]
        ...
        # target network
        target_q_n = [q_func(q_input, 1, scope="w_target_q_func_{}".format(i), num_units=num_units)[:,0] for i in range(num_agents)]
        target_q_func_vars = [U.scope_vars(U.absolute_scope_name("w_target_q_func_{}".format(i))) for i in range(num_agents)]
...
class SchednetTrainer(AgentTrainer):
...
#            network_input=network_obs,
        ...
        self.replay_buffer = ReplayBuffer_W(1e6)
    ...
    def update(self, agents, t):
        ...
        return [q_loss, p_loss, w_q_loss, w_p_loss]
